...
class resnet_C3Rcbam(Network):
    ...
    def cbam_module(self, inputs, reduction_ratio=0.5, name=''):
        with tf.variable_scope("cbam_" + name, reuse=None):
            ...
            mlp_1_max = slim.fully_connected(inputs=maxpool_channel, num_outputs=int(hidden_num * reduction_ratio),
                                             scope="mlp_1",
                                             reuse=None)
            ...
            mlp_1_avg = slim.fully_connected(inputs=avgpool_channel, num_outputs=int(hidden_num * reduction_ratio),
                                             scope="mlp_1",
                                             reuse=True)
    ...
    def build_network(self, sess, is_training=True):
        ...
        with tf.variable_scope('noise'):
            ...
            conv = slim.conv2d(self.noise, num_outputs=3, kernel_size=[5, 5], stride=1, padding='SAME',
                               activation_fn=None, trainable=is_training, scope='constrained_conv')
        ...
        if cfg.USE_MASK is True:
            ...
            with tf.variable_scope('mask_predict', 'mask_predict'):
                ...
                upsampled_features = slim.conv2d(upsampled_features, 64, [1, 1], normalizer_fn=slim.batch_norm,
