...
def im_detect(sess, net, im):
    ...
    if cfg.USE_MASK is True:
        blobs['im_info'] = np.array([[im_blob.shape[1], im_blob.shape[2], im_scales[0], im.shape[0], im.shape[1]]],
                                    dtype=np.float32)
        scores1, scores, bbox_pred, rois, feat, s, y_preds, mask_data, layers = net.test_image(sess, blobs['data'],
                                                                                               blobs['noise'],
                                                                                               blobs['im_info'])
    ...
    if os.path.isfile(os.path.join(output_dir, 'detections.pkl')):
        ...
        if cfg.USE_MASK is True:
            ...
            for i in range(num_images):
                ...
                scores, boxes, feat, s, maskcls_inds, mask_boxes, mask_scores, mask_pred, _, _ = im_detect(sess, net,
                                                                                                           im)
